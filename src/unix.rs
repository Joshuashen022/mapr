...
impl MmapInner {
    ...
    pub fn new_from_c_void(ptr: *mut libc::c_void, len: usize) -> io::Result<MmapInner> {
        if ptr == libc::MAP_FAILED {
            Err(io::Error::last_os_error())
        } else {
            Ok(MmapInner { ptr, len: len })
        }
    }
    ...
    /// SN-lotus hugepage memory feature.
    /// Open an anonymous memory map.
    pub fn map_anon_hugepage(
        len: usize,
        stack: bool,
        locked: bool,
        private: bool,
