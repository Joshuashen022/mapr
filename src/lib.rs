...
impl MmapOptions {
    ...
    pub fn map_anon(&self) -> Result<MmapMut> {
        MmapInner::map_anon(self.len.unwrap_or(0), self.stack, self.locked, self.private)
            .map(|inner| MmapMut { inner: inner })
    }

    /// For sn-lotus hugepage memory feature
    /// This is the entrance
    pub fn map_anon_hugepage(&self) -> Result<MmapMut> {
        MmapInner::map_anon_hugepage(self.len.unwrap_or(0), self.stack, self.locked, self.private)
            .map(|inner| MmapMut { inner: inner })
    }
}
...
impl MmapMut {
    ...
    pub unsafe fn create_mmp(mem_size: usize, huge_page: bool) -> Result<MmapMut> {
        ...
        let ptr_u8: *mut u8 =
            simple_alloicate_memory_address(mem_size, huge_page, None).unwrap() as *mut u8;
